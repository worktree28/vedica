...
VITE_GITHUB_USER=hmishra
VITE_GITHUB_REPO=vedica
VITE_GITHUB_BRANCH=main
```

### Update imageConfig.js
Make sure to update the GitHub repository information in `src/config/imageConfig.js`:
```javascript
github: {
...
2. ✅ Update GitHub info in `src/config/imageConfig.js`
3. ✅ Set `VITE_IMAGE_MODE=JSDELIVR` in production environment
4. ✅ Test image loading in production build locally:
   ```bash
   VITE_IMAGE_MODE=JSDELIVR npm run build
   npm run preview
   ```
5. ✅ Verify all images load correctly from jsDelivr
6. ✅ Check browser console for any 404 errors
7. ✅ Test on different devices and network speeds
